...
int main(int argc, char *argv[]) {
    ...
    srand(time(NULL) + id); // Seed the random number generator for Monte Carlo

    // Loop to search for the key using Monte Carlo approach
    for (long attempts = 0; attempts < (myupper - mylower); ++attempts) {
        ...
        long random_key = mylower + rand() % (myupper - mylower + 1);

        if (tryKey(random_key, cipher, ciphlen, search)) {
            found = random_key; // Key found
